...
- The restarted **list** row title can briefly show the projection-cache checkpoint title: the cache (`packages/session/session-projection-cache`; web-app row config `writeEveryEvents: 200`, `writeIntervalMs: 5000`) checkpoints at the last `turn/end` or session disposal, and a rename that lands after the last `turn/end` is absent from the cold-start row. Opening the session replays the log tail and the row relabels to the user rename; no data loss.
- The restarted client can log one transient `[cordis-client-runner] syncing inspect providers failed: … no active Connection` while its inspect sync loses the connection bring-up race.

Both are race-window / cache-staleness concerns owned by Stage 8 (deferred: race windows, event loss, reconnect pathology, pending interactions across renderer reload, adversarial disconnect), not stage 6 scope. The restart test asserts the true parity outcome — sessions recovered, nothing running, history reopens, the row relabels to the user rename on open, the rename durable in the log, no welcome re-shown — and fails on any other renderer error, with exactly that one transient allowlisted by message prefix.
...
- Two DSH findings to report upstream: projection-cache cold-start title staleness (self-corrects when the session opens) and the cold-start inspect-sync race.
