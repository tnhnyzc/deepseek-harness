...
| 插件释放 | 被等待的排空——先 settle 在途写入，再持久 flush 所有仍脏的会话，最后关闭 domain；排空注册了关闭延迟（`Domain.deferClose`），设施的卸载 `closeAll` 与已路由后端自身的 `close()` 都不会先落地并拒绝其最终写入。因此干净关机绝不会留下最后一个检查点（例如末次写入之后的重命名）未落盘、冷列表读到陈旧行。 |
